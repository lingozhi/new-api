...
export function buildMiniMaxVideoParameters(
  ...
  return [
    ...
    },
    {
      ...
      type: 'string',
      descriptionKey:
        'Reserved by MiniMax V2; non-empty callbacks are not supported by this AutoDL backend',
    },
    {
      name: 'aigc_watermark',
      type: 'boolean',
...
export function buildMiniMaxVideoAiIntegrationGuide(
  ...
  return [
    ...
    '- first_frame, last_frame, reference_video, non-empty callback_url, and aigc_watermark=true are unsupported.',
    '',
    '## Async task lifecycle',
    '- A successful submit returns HTTP 200 with {"task_id":"task_..."}. It is not an HTTP 202 contract.',
    '- Poll the query endpoint with the same user token until queued or running becomes succeeded, failed, or cancelled.',
