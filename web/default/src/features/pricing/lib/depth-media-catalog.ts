...
export function buildDepthMediaJobSample(
  ...
  if (language === 'python') {
    return [
      ...
      `response = requests.post("${submitUrl}", headers=headers, json=${JSON.stringify(payload)}, timeout=30)`,
      ...
      '    if remaining <= 0:',
      '        raise TimeoutError("Task polling timed out after 900 seconds")',
      '    request_timeout = max(1, min(30, remaining))',
      `    result_response = requests.get(f"${submitUrl}/{task['task_id']}", headers=headers, timeout=request_timeout)`,
  ...
  return [
    ...
    `let result`,
    `while (true) {`,
    `  const pollResponse = await fetch(\`${submitUrl}/\${task.task_id}\`, {`,
    `    headers,`,
    `    signal: AbortSignal.timeout(30_000),`,
    `  })`,
    ...
    `  if (Date.now() >= deadline) throw new Error('Task polling timed out after 900 seconds')`,
