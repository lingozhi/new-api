...
export function buildIndexTTSAudioSpeechParameters(
  ...
  return [
    ...
    },
    {
      ...
      defaultValue: 'wav',
      enumValues: ['wav'],
      descriptionKey: 'Generated audio format; only wav is supported',
    },
    {
      name: 'speed',
      type: 'number',
      defaultValue: 1,
      enumValues: ['1'],
      descriptionKey: 'Speech speed; IndexTTS2 currently requires 1',
    },
    {
      name: 'metadata',
      type: 'object',
      descriptionKey:
        'Optional emotion controls: emotion_audio, emotion_vector, or emotion_random',
    },
  ]
}
  ...
  return JSON.stringify(
    {
      ...
      input: '你好，这是一段 IndexTTS2 语音合成示例。',
      voice: REFERENCE_AUDIO_PLACEHOLDER,
      response_format: 'wav',
      speed: 1,
    },
    null,
...
function buildCurlSample(context: AudioSpeechSampleContext): string {
  ...
  return [
    `# Set ${context.apiKeyEnv}; replace <UNIQUE_ID> and <REFERENCE_AUDIO_URL>.`,
...
function buildPythonSample(context: AudioSpeechSampleContext): string {
  ...
  return [
    ...
    `    "model": "${context.modelName}",`,
    '    "input": "你好，这是一段 IndexTTS2 语音合成示例。",',
    `    "voice": "${REFERENCE_AUDIO_PLACEHOLDER}",  # Replace before running.`,
    '    "response_format": "wav",',
    '    "speed": 1,',
    '}',
    '',
...
export function buildIndexTTSAudioSpeechAiIntegrationGuide(
  ...
  return [
    ...
    '- The JSON body must contain model, input, and voice. Use response_format=wav and speed=1.',
    '- voice is reference speaker audio, not a named OpenAI voice. It accepts a public HTTPS WAV/MP3 URL or a matching base64 data URI.',
    '',
    ...
    '',
    '## Limits',
    '- input must contain 1 to 2048 UTF-8 characters.',
    '- The complete JSON request body is limited to 64 MiB.',
    '- Each reference audio input is limited to 15 MiB; voice plus emotion_audio is limited to 30 MiB decoded.',
    '- Reference WAV/MP3 duration is limited to 10 minutes. Generated WAV output is limited to 64 MiB.',
    '',
    '## Emotion metadata',
    '- metadata.emotion_audio accepts the same public HTTPS/data-URI audio formats as voice.',
    '- metadata.emotion_vector contains exactly eight values in this order: happy, angry, sad, afraid, disgusted, melancholic, surprised, calm.',
    '- Every vector value must be between 0 and 1.4. The surprised value currently must be 0.',
    '- emotion_audio, emotion_vector, and emotion_random=true are mutually exclusive.',
    '',
    '## Unsupported options',
