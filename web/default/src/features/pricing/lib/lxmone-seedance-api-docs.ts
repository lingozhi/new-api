...
export function buildLxmoneSeedanceAiIntegrationGuide(
  ...
  return [
    ...
    '- Media URLs must be accessible to the provider, not localhost/private-only files. Replace example.com placeholders with real assets. The gateway validates counts and URL syntax, including matching frame aliases. The provider validates public reachability, actual formats and media duration/size. Object inputs must contain only url and are normalized to URL strings.',
