...
export function buildLxmoneSeedanceAiIntegrationGuide(
  ...
  return [
    ...
    '- Use JSON requests and public media URLs. The old /v1/media/uploads and /v1/videos/generations workflow is not the documented integration for this channel.',
    '',
    '## Models and limits',
    ...
    'Omit unused optional fields; do not send null, empty placeholders or every optional field at once. Use one spelling per alias pair.',
    ...
    '| input_reference | string URL | optional | First-frame image URL. Cannot be combined with reference lists. |',
    '| image | string URL | optional | Alternative first-frame field; use instead of input_reference. |',
    '| image_end | string URL | optional | Last-frame image URL; requires a first frame. |',
    '| end_image_url | string URL | optional | Alternative last-frame field; use instead of image_end. |',
    '| reference_images | array of {"url": string} | optional; omitted | Reference image URLs, in order. |',
    '| reference_videos | array of {"url": string} | optional; omitted | Reference video URLs. |',
    '| reference_audios | array of {"url": string} | optional; omitted | Reference audio URLs. See audio dependency below. |',
    '',
    ...
    '- Provider-advertised maximum counts: Seedance 2/Fast/Mini 9 images, 3 videos, 3 audios; Seedance 2.5 30 images, 10 videos, 10 audios.',
    '- Media URLs must be accessible to the provider, not localhost/private-only files. Replace example.com placeholders with real assets. The provider validates file availability, formats, media duration/size and counts; do not infer unlimited support from gateway passthrough.',
    ...
    '```',
    '- A completed query uses the same public id/model with status="completed", progress=100. Use the authenticated content endpoint rather than assuming a video.url field exists.',
    ...
    '- video_generation_failed: inspect the saved task error; do not run automatic paid retries.',
