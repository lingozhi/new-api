...
export function buildWanAiIntegrationGuide(origin: string): string {
  ...
  return [
    ...
    '',
    '## Current official channel: modes',
    '| mode | Required input |',
    '| --- | --- |',
    '| general | prompt; reference lists optional |',
    '| reference | prompt + at least one reference image, video or audio |',
    '| frames | prompt + first_frame + last_frame |',
    ...
    '- Keep model=wan3.0 for every mode. speed is not supported; omit it. speed and reference-video duration metadata return HTTP 400 before billing. Retired provider model IDs are unsupported.',
    '',
    ...
    '```',
    'Use wan3.0 and the supported fields above. Retired channel parameter and model compatibility has been removed.',
  ].join('\n')
}
